...
class FPS
  ...
  def log_fps(aplog = "", pid = "", length = 0)
    ...
    if File.readable_real?(aplog) && ! pid.empty? && length > 0
      ...
      if alive?(head,tail).is_a?(Array) && ! alive?(head,tail).empty?
        fps_arr.concat(alive?(head,tail))
      end
      ...
      fps_text.each { |t| fps_set << t.split()[-1] } 
      fps_min = fps_set.min
      fps_max = fps_set.max
  ...
  def readfps(fps)
    if fps.is_a? Array
      name = %w(instance pid start end cost now samples min max sum avg)
